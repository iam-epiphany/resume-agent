...
def test_parse_markdown_returns_structured_blocks(tmp_path) -> None:
    ...
    path.write_text("# 简历填写说明\n\n## 技能专长\n技能专长应涵盖 Java 与 Python 两个方向。\n", encoding="utf-8")
...
def test_parse_html_preserves_headings_paragraphs_and_tables_as_plain_text(tmp_path) -> None:
    ...
    path.write_text(
        "<html><head><title>简历材料</title></head><body><h1>第一章</h1><p>正文依据。</p>"
        "<table><tr><th>指标</th><th>值</th></tr><tr><td>技能</td><td>12</td></tr></table></body></html>",
        encoding="utf-8",
    )

    parsed = parse_document(path)

    assert parsed.metadata["html_title"] == "简历材料"
    assert any(block.block_type == "heading" and block.text == "第一章" for block in parsed.blocks)
    assert any(block.block_type == "paragraph" and "技能" in block.text for block in parsed.blocks)
...
def test_chunks_inherit_section_from_structured_blocks(tmp_path) -> None:
    ...
    assert len(chunks) == 1
    assert chunks[0].section_title == "技能专长"
    assert "技能专长应涵盖 Java 与 Python 两个方向" in chunks[0].text
    assert "章节路径：简历填写说明 > 技能专长" in chunks[0].embedding_text
    assert "章节：技能专长" in chunks[0].embedding_text
    ...
    parsed = ParsedDocument(
        ...
        blocks=[
            ParsedBlock(
                ...
                order_index=1,
                page_number=7,
                section_title="3.1 技能专长差异处理",
                level=2,
            ),
            ParsedBlock(
                text="若方向侧重不同，应保留各自的项目经历。",
...
def test_chunk_splitting_respects_max_tokens_and_overlap() -> None:
    ...
    parsed = ParsedDocument(
        text=text,
        blocks=[ParsedBlock(text=text, block_type="paragraph", order_index=1, section_title="技能专长")],
    )
...
def test_long_paragraph_prefers_sentence_boundaries() -> None:
    sentence_a = "技能专长应涵盖 Java 与 Python 两个方向。"
...
def test_semantic_break_splits_topic_shift(monkeypatch) -> None:
    paragraphs = [
        "技能专长应涵盖 Java 与 Python 两个方向。" * 80,
        "技能分类应按照简历规定保持表述一致。" * 80,
        "成绩排名指标应按照简历口径计算。" * 80,
    ]
    parsed = ParsedDocument(
        text="\n\n".join(paragraphs),
...
def test_docx_loader_preserves_paragraph_and_table_order_as_plain_text(tmp_path) -> None:
    ...
    table.cell(1, 0).text = "技能专长"
    table.cell(1, 1).text = "技能掌握程度"
    ...
    assert "技能专长" in parsed.blocks[1].text
    assert "技能掌握程度" in parsed.blocks[1].text
...
def test_pdf_defaults_to_pymupdf4llm_loader(tmp_path) -> None:
    ...
    page.insert_text((72, 72), "Total skills equal the sum of asset items.")
    document.save(path)
    document.close()
    ...
    assert "Total skills" in parsed.text
